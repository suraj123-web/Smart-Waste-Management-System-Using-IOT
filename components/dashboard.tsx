...
import type { DashboardTelemetry } from "../lib/blynk";
...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadTelemetry() {
      const response = await fetch("/api/blynk/status", { cache: "no-store" });
      if (!response.ok) {
        setConnected(false);
        return;
      }

      const data = (await response.json()) as DashboardTelemetry;
      if (active) {
        setTelemetry(data);
        setConnected(data.source === "blynk");
      }
    }
  ...
  const kpiCards = useMemo(
    () => [
      {
        title: "Total Bins",
        value: String(telemetry.totalBins),
        trend: connected ? "Live from Blynk" : "Fallback data",
        icon: Trash2,
      },
      {
        title: "Collections Completed",
        value: String(telemetry.collectionsCompleted),
        trend: connected ? "Live from Blynk" : "Fallback data",
        icon: CheckCircle2,
      },
      {
        title: "Average Fill Level",
        value: `${telemetry.averageFillLevel}%`,
        trend: connected ? "Live from Blynk" : "Fallback data",
        icon: BarChart3,
      },
      {
        title: "Critical Alerts",
        value: String(telemetry.criticalAlerts),
        trend: connected ? "Live from Blynk" : "Fallback data",
        icon: AlertTriangle,
      },
    ],
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-8 lg:px-14">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <p className="inline-flex items-center gap-2 text-xs text-slate-300">
              ...
              {connected ? "Connected to Blynk Cloud" : "Blynk unavailable (using fallback)"}
            </p>
          </div>
        ...
        <footer className="flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/50 p-4 text-sm text-slate-300">
          ...
          <p>Last synchronized: {new Date(telemetry.lastSync).toLocaleTimeString()}</p>
        </footer>
      </div>
    </div>
  );
